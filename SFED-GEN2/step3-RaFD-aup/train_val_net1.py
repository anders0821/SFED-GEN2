...
def build_graph(mode):
    ...
    with tf.variable_scope('CONV-7') as ns:
        mlp = conv(mlp, 512+7, name='conv71')
        mlp = bn(mode, mlp)
        mlp = lrelu(mlp)
        mlp = pool(mlp)
        y_hat_logit = mlp[:, :, :, 512:]
    ...
    with tf.variable_scope('L_REC'):
        z_hat = mlp
        del mlp
        l_rec = tf.nn.l2_loss(z - z_hat) / tf.cast(tf.shape(x)[0], tf.float32)

    # FC分类分支
    with tf.variable_scope('L_CLS'):
        y_hat_logit = tf.squeeze(y_hat_logit, [1, 2])
        l_cls = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(y_hat_logit, y))
        acc = tf.equal(tf.argmax(y_hat_logit, 1), tf.argmax(y, 1))
        acc = tf.reduce_mean(tf.cast(acc, tf.float32))
        y_hat = tf.nn.softmax(y_hat_logit)

    with tf.variable_scope('L'):
        LAMBDA_1 = 1
        l = l_rec + LAMBDA_1 * l_cls
    ...
    return [x, y, z, l_rec, l_cls, l, z_hat, y_hat, acc, train_op]
...
def main():
    ...
    with tf.Graph().as_default():
        ...
        with tf.variable_scope('GRAPH', reuse=None):
            [train_x, train_y, train_z, train_l_rec, train_l_cls, train_l, train_z_hat, train_y_hat, train_acc, train_op] = build_graph('train')
        with tf.variable_scope('GRAPH', reuse=True):
            [val_x, val_y, val_z, val_l_rec, val_l_cls, val_l, val_z_hat, val_y_hat, val_acc, _] = build_graph('val')
        ...
        with tf.Session() as sess:
            ...
            for epoch in xrange(SNAPSHOT_RESUME_FROM + 1, EPOCH_MAX + 1):
                ...
                mean_train_l_rec = 0.0
                mean_train_l_cls = 0.0
                mean_train_l = 0.0
                mean_train_acc = 0.0
                mean_train_count = 0
                ...
                for itr in xrange(ITER_COUNT):
                    ...
                    # run
                    [_, train_l_rec_val, train_l_cls_val, train_l_val, train_z_hat_val, train_y_hat_val, train_acc_val] = \
                        sess.run([train_op, train_l_rec, train_l_cls, train_l, train_z_hat, train_y_hat, train_acc], feed_dict={train_x: train_x_val, train_y: train_y_val, train_z: train_z_val})
                    mean_train_l_rec += train_l_rec_val * MB
                    mean_train_l_cls += train_l_cls_val * MB
                    mean_train_l += train_l_val * MB
                    mean_train_acc += train_acc_val * MB
                    mean_train_count += MB
                    # print train_l_rec_val, train_l_cls_val, train_acc_val
                print 'mean_train_l_rec %g, mean_train_l_cls %g, mean_train_l %g, mean_train_acc %g' % \
                      (mean_train_l_rec / mean_train_count, mean_train_l_cls / mean_train_count, mean_train_l / mean_train_count, mean_train_acc / mean_train_count)
                ...
                if (epoch % SNAPSHOT_INTERVAL) == 0:
                    ...
                    for i in xrange(3):
                        ...
                        plt.imshow((train_z_hat_val[i, :, :, :].squeeze().T * STD) + MEAN, cmap='gray', vmin=0.0, vmax=255.0)
                        plt.axis('off')
                        plt.title(train_y_hat_val[i, :])
                        ...
                        plt.imshow((val_z_hat_val[i, :, :, :].squeeze().T * STD) + MEAN, cmap='gray', vmin=0.0, vmax=255.0)
                        plt.axis('off')
                        plt.title(val_y_hat_val[i, :])

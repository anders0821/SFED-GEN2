...
def main():
    ...
    # GEN
    SUB = 12
    TRAIN_X = TRAIN_X[7*SUB+3:7*SUB+3 + 1, :, :, :]
    TRAIN_Z = TRAIN_Z[7*SUB+3:7*SUB+3 + 1, :, :, :]
    TRAIN_X = np.tile(TRAIN_X, [7, 1, 1, 1])
    TRAIN_Z = np.tile(TRAIN_Z, [7, 1, 1, 1])
    TRAIN_Y = np.diag(np.ones(7, np.uint8))
    VAL_X = VAL_X[7*SUB+3:7*SUB+3 + 1, :, :, :]
    VAL_Z = VAL_Z[7*SUB+3:7*SUB+3 + 1, :, :, :]
    VAL_X = np.tile(VAL_X, [7, 1, 1, 1])
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as sess:
            ...
            for itr in xrange(ITER_COUNT):
                ...
                mb = min(itr * MB + MB, VAL_X.shape[0]) - itr * MB
                val_x_val = VAL_X[itr * MB:itr * MB + mb, :, :, :]
                val_y_val = VAL_Y[itr * MB:itr * MB + mb, :]  # GEN
